...
exports.lambdaHandler = async (event, context) => {
  ...
  const getUserQuery = `SELECT * FROM Listing WHERE ListingID = ${ListingID}`;
  ...
  if (getUser.length < 1) {
    return {
      statusCode: 404,
      body: `Requested Deletion of User with UserID ${ListingID} not found`,
    };
  }

  if (
    parseInt(getUser[0]['UserID']) !== parseInt(UserID) &&
    userPrivileges === 0
  ) {
    return {
      statusCode: 403,
